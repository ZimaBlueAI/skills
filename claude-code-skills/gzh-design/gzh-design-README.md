...
把 Markdown / Word / PDF / 纯文本一键排成**可直接粘贴进微信公众号编辑器、粘贴后样式不丢失**的 HTML。移植自 [isjiamu/gzh-design-skill](https://github.com/isjiamu/gzh-design-skill)（AGPL-3.0，甲木 × 摸鱼小李联名共建），来源与升级方式见 `NOTICE.md`。
...
| 能力 | 说明 |
|---|---|
| 6 套内置主题 | 摸鱼绿（默认）· 红白色系 · 石墨极简 · 留白禅意 · 摸鱼票据 · 橄榄手记，每套自成体系（设计变量 + 数十组件 + 骨架 + 文章类型配方表），清单见 `references/theme-index.md` |
...
| 一键复制预览 | `scripts/wrap_preview.py` 生成带「复制到公众号」按钮的预览页，点一下把富文本进剪贴板 |
...
├── references/                 # 主题索引 + 6 套主题库 + 通用库 + 生成器 + 归一化 + 回归用例
├── scripts/                    # component_lint / validate_gzh_html / extract_docx / wrap_preview
├── assets/                     # 预览模板 + 样例文章 + 主题预览示例
