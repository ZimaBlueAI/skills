...
# 频道对话 · 做图做PPT并发回当前会话 (channel delivery)

> **一句话**:用户在 IM 频道里说一句需求,你(OpenClaw)把需求问清 → 调用
...
> 架构上可扩展——加企业微信 / Slack / Telegram 只需在 `scripts/channel_deliver.py`
> 里加对应 adapter,频道对话与生成逻辑完全复用。下文以飞书为例。
...
装法见同目录 `install-openclaw-skills.ps1` / `.sh`。
...
- OpenClaw 的飞书 channel 会把当前会话上下文给到你。当前 chat_id 通常在环境变量里。
...
- 平台与 chat_id 细节见 `references/openclaw-channel.md`。
