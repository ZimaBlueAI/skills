...
### 案例 10 · 公众号文章排版(gzh-channel,先确认再排)
- 🧑(发来一篇长文或 .docx)「排成公众号发我」
- 机器人:整理成 Markdown 结构稿 → 发一张 **interactive 卡片**(标题/章节/字数摘要 +
  Markdown 全文代码块,长稿则附 `.md` 文件)→ 你回「确认」或说改哪里
- 🧑「确认」→ 机器人问一句主题(默认推荐摸鱼绿,也可红白/石墨极简/留白禅意/摸鱼票据/橄榄手记)
- 📦 两个文件:`*_预览.html`(浏览器打开 → 点右上角「**复制到公众号**」→ 编辑器粘贴,
  样式不丢)+ 干净正文 HTML(手动粘贴兜底)。
- ✦ 关键点:**排版前必过草稿确认卡片**——章节切错排完全废;排版由 `gzh-design`
  完成(样式全内联 + `<span leaf>` 包裹 + 校验 ERROR×0),粘贴到公众号编辑器不掉格式。
