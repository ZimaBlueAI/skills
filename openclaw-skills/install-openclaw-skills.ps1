<#
OpenClaw · ZimaBlueAI 动效HTML+PPTX+频道对话交付 一键安装 (Windows / PowerShell)
================================================================================
把生成能力(viz-deck / viz-charts [/ biz-html-viz])+ 频道对话交付技能
(viz-channel)装进 OpenClaw 的 skills 目录,配齐依赖。

生成能力本体从【本仓库 claude-code-skills\】的 zip 取(默认),或 -FromGithub 实时拉。

用法:
  ./install-openclaw-skills.ps1
  ./install-openclaw-skills.ps1 -Target "$HOME/.openclaw/skills"
  ./install-openclaw-skills.ps1 -WithBridges        # huashu + ppt-master(MP4 / 可编辑 PPTX)
  ./install-openclaw-skills.ps1 -WithBiz            # 额外装 biz-html-viz
  ./install-openclaw-skills.ps1 -Minimal           # 只装动效 HTML 所需
  ./install-openclaw-skills.ps1 -FromGithub
#>
[CmdletBinding()]
param(
    [string]$Target = "$HOME\.openclaw\skills",
    [switch]$WithBridges,
    [switch]$WithBiz,
    [switch]$Minimal,
    [switch]$FromGithub
...
$Tmp = Join-Path $env:TEMP "openclaw-zima-install-$PID"
$Extract = Join-Path $Tmp "extracted"
$Archive = Join-Path $Target ".zima-replaced"

Head "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
Head "  OpenClaw · 动效HTML + PPTX + 频道对话交付"
Head "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
Write-Host ""
...
# [4] 依赖
Head "[4/5] 依赖"
try { python -m pip install --quiet requests pyyaml; Ok "python: requests + pyyaml" }
...
Write-Host "细节见: $Target\viz-channel\references\openclaw-channel.md"
