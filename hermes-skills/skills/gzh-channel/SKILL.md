...
description: >-
  ...
  公众号 HTML → 把带「复制到公众号」按钮的预览页发回当前 chat,用户本地打开一键复制粘贴。
  生成靠 gzh-design,草稿确认与送达靠本技能。触发词:公众号排版、微信排版、排成公众号、
  公众号文章、gzh、排版发我、发公众号。
license: MIT
---

# 频道对话 · 公众号排版并发回当前会话 (gzh channel delivery · Hermes)

> **一句话**:用户在飞书频道里丢来一篇文章,你(Hermes Agent)把它整理成 Markdown 草稿
> → 用 **interactive 卡片(代码块形式)** 发回让用户确认 → 确认后调 `gzh-design`
> 排版成公众号 HTML → 用 `gzh_send.sh` 把**带「复制到公众号」按钮的预览页**发回
...
装法见同目录 `install-hermes-skills.sh` / `.ps1`(默认一并安装)。
...
(`FEISHU_CHAT_ID` / `HERMES_CHAT_ID` 等,脚本自动解析);解析不到时从这条
飞书消息的事件上下文里取 `chat_id`(群是 `oc_` 开头),交付时用
`--to <chat_id> --to-type chat_id` 显式传。平台细节见 viz-channel 的
`references/hermes-channel.md`(两技能共用同一套 chat 解析)。
...
`wrap_preview.py` 生成带「复制到公众号」按钮的 `_预览.html`。
...
bash scripts/gzh_send.sh \
  ...
  --to-current \
  --note "公众号排版好了 ✦ 下载「_预览.html」用浏览器打开 → 点右上角「复制到公众号」→ 到公众号编辑器 Ctrl/⌘+V 粘贴即可,样式不会丢。另一个文件是干净正文,手动粘贴兜底用。"
```
...
- 「点右上角**复制到公众号** → 公众号编辑器粘贴」——这是整条链路的终点动作。
