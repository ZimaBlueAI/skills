#!/usr/bin/env bash
# Hermes Agent · ZimaBlueAI 动效HTML+PPTX+频道对话交付 一键安装
# =============================================================
# 把生成能力(viz-deck / viz-charts [/ biz-html-viz])+ 频道对话交付技能(viz-channel)
# 装进 Hermes 的 skills 目录,配齐依赖。
#
# 生成能力本体从【本仓库 claude-code-skills/】的 zip 取(默认),或 --from-github 实时拉。
# 频道交付技能(viz-channel)来自本目录 skills/。当前投递 adapter = 飞书/Lark。
#
# Hermes 的 skills 目录因部署而异,默认 ~/.hermes/skills。先 `hermes doctor` 或看配置确认;
# 不对就用 --target <你的skills目录> 指定。
#
# 用法:
#   bash install-hermes-skills.sh                      # 默认装到 ~/.hermes/skills
#   bash install-hermes-skills.sh --target ~/.hermes/skills
#   bash install-hermes-skills.sh --with-bridges       # 连 huashu + ppt-master(MP4 / 可编辑 PPTX)
#   bash install-hermes-skills.sh --with-biz           # 额外装 biz-html-viz
#   bash install-hermes-skills.sh --minimal            # 只装动效 HTML 所需
#   bash install-hermes-skills.sh --from-github        # 生成技能从 GitHub 拉而非本仓库 zip

set -u
GREEN='\033[0;32m'; YELLOW='\033[1;33m'; RED='\033[0;31m'; BLUE='\033[0;34m'; RESET='\033[0m'

TARGET="$HOME/.hermes/skills"
WITH_BRIDGES=0; WITH_BIZ=0; MINIMAL=0; FROM_GITHUB=0
...
TMP="${TMPDIR:-/tmp}/hermes-zima-install-$$"

echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
echo -e "${BLUE}  Hermes · 动效HTML + PPTX + 频道对话交付${RESET}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
echo
echo "目标目录: $TARGET"
...
command -v hermes >/dev/null 2>&1 \
    && echo -e "  ${GREEN}✓${RESET} hermes ($(hermes --version 2>&1 | head -1))" \
    || echo -e "  ${YELLOW}⚠${RESET} 未在 PATH 找到 hermes(若 skills 目录已确认仍可继续)"
echo

# ─── [2] 取得生成技能的 zip ───
echo -e "${BLUE}[2/5] 准备生成技能 (viz-deck / viz-charts$([ $WITH_BIZ -eq 1 ] && echo ' / biz-html-viz'))${RESET}"
mkdir -p "$TARGET"
EXTRACT="$TMP/extracted"; mkdir -p "$EXTRACT"
...
if [ $WITH_BIZ -eq 1 ]; then
    ...
    if [ -d "$EXTRACT/.claude/agents" ]; then
        ...
        echo -e "  ${GREEN}✓${RESET} biz subagents → $AG (Hermes 子技能/Hub 可选)"
    fi
fi
echo
...
pip3 install --quiet requests pyyaml 2>/dev/null \
    && echo -e "  ${GREEN}✓${RESET} python: requests + pyyaml(频道投递·飞书adapter)" \
...
echo
echo "Hermes 会:读 viz-channel → 频道里问清需求 → viz-deck 模式1 生成动效HTML"
echo "        → channel_send.sh 发回当前会话。也可做成 cron/webhook 自动化。"
echo "细节见: $TARGET/viz-channel/references/hermes-channel.md"
