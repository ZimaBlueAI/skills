...
$homeAgents = Join-Path $HOME ".agents\skills"
...
Write-Host "[codex-skills] target: $homeAgents + $homeCodexAgents"
Invoke-Step { New-Item -ItemType Directory -Force -Path $homeAgents | Out-Null }
...
Write-Host "  1) In Codex CLI run /skills to confirm the three skills appear"
Write-Host "  2) (Optional) Install ppt-master to unlock v3 editable-PPTX:"
Write-Host "       git clone https://github.com/hugohe3/ppt-master `"$homeAgents\ppt-master`""
Write-Host "       cd `"$homeAgents\ppt-master`"; python -m venv .venv"
...
Write-Host "       git clone https://github.com/alchaincyf/huashu-design `"$homeAgents\huashu-design`""
