...
### zima-design（设计合奏引擎 · ZimaBlueAI 自有）

原创路由协议编排三个内置引擎：anti-slop（21 宏结构 × 20 主题 × 57 条 slop-test 关卡，源自 nutlope/hallmark）+ motion（动画决策框架 / 弹簧物理 / 性能铁律，源自 emilkowalski/skills）+ database（84 风格 / 192 配色 / 98 UX 准则本地检索，源自 ui-ux-pro-max，需 Python 3）。五轴多样性协议保证连续交付不共享模板骨架。用于落地页 / 官网 / 产品 UI 的从零设计、反 AI 味审计与重设计、动效专项、选型检索；本套件各生成 skill 已内联其六大纪律硬闸，装了 zima-design 自动桥接全量关卡。
...
unzip -o codex-skills/zima-design/zima-design.zip -d /path/to/project/
```
...
应能看到 `biz-html-viz` · `viz-deck` · `viz-charts` · `zima-html-ppt` · `gzh-design` · `zima-design`。
...
```bash
ls -la ~/.codex/skills/         # 应有 biz-html-viz / viz-deck / viz-charts / zima-html-ppt / gzh-design / zima-design
