...
v0.4.0 起共三个 skill，覆盖**从董事会简报到对外讲演视频再到真正可编辑 PPT**的全链路；**Claude Code 与 OpenAI Codex CLI 双 harness 并发布**。所有 skill 相互独立，按需取用。
...
上面三个 skill 只管**生成**。v0.5 在 **OpenClaw** 与 **Hermes** 两个 IM-gateway agent 上补齐了**对话编排 + 频道交付**——新增 **`viz-channel`** 技能,让用户在飞书等频道里说一句话,agent 就**问清需求 → 调 viz-deck/viz-charts 生成 → 把成品作为附件发回当前会话**。
...
| | 说明 |
|---|---|
| **默认风格** | **ZimaBlue Editorial(暖纸编辑风)**——暖白纸底 + 深青主色 + 金/赭点缀 + 发丝卡片 + 顶部三色条,专业简洁出片即用(可一句话切换);见 `viz-channel/references/default-style.md` |
